...
fun MoviePlayer(movie: Movie){
    Divider(modifier = Modifier.padding(3.dp))
    Text(text = "Movie Trailor")
    var lifecycle by remember {
        mutableStateOf(Lifecycle.Event.ON_CREATE)
    }
    val context = LocalContext.current
    val mediaItem = MediaItem.fromUri(
        Uri.parse("android.resource://${context.packageName}/"+context.resources.getIdentifier(movie.trailer, "raw", context.packageName)))
    ...
    Modifier.fillMaxWidth().aspectRatio(18f/9f),
        factory = {
            PlayerView(context)
    ...
    , update =
        {
            when (lifecycle) {
                Lifecycle.Event.ON_RESUME -> {
                    it.player?.play()
                    Log.i("Player", "onRESUME called.")
                }
                Lifecycle.Event.ON_PAUSE -> {
                    it.player?.pause()
                    Log.i("Player", "onPAUSE called.")
                }
                Lifecycle.Event.ON_STOP -> {
                    it.player?.stop()
                    Log.i("Player", "onSTOP called.")
                }
                else -> Unit
            }
        }
